...
* **`skipDefiningReporter`** — Node-only examples (`node_example`, `node_tap_example`, `node_test`, `vitest`) and `electron`. The runner otherwise appends `--launch "Headless Firefox"`, which these examples do not use.
