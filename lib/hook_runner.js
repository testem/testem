...
HookRunner.prototype = {
  run: function(hook, data, callback) {
    ...
    if (command) {
      ...
      this.processCtl.exec(command, options);
    } else if (exe) {
      args = this.varsub(args || []);
      this.processCtl.spawn(exe, args, options);
    ...
    this.processCtl.on('processExit', function(code, stdout, stderr) {
      if (code !== 0) {
    ...
    this.processCtl.on('processError', function(code, err, stdout, stderr) {
      finished(err, stdout, stderr);
    });

    if (waitForText) {
      this.processCtl.onStdOut(this.varsub(waitForText), finished, waitForTextTimeout);
    }
    if (badText) {
      this.processCtl.onStdOut(this.varsub(badText), function(err, stdout, stderr) {
        if (err) {
