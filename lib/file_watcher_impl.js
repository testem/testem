 ...
 *
 * **Third-party pieces in use today:** `chokidar` (filesystem watch engine). Glob matching and
 * magic detection live in {@link module:lib/utils/path_pattern_match} (picomatch). Policy
 ...
 * {@link FileWatcherImpl.createWatcher} (with `chokidar` loaded lazily there), and glob
 * matching is confined to small helpers—so swapping the watcher or glob detection is
...
const { patternHasGlobMagic } = require('./utils/path_pattern_match');
const {
...
function includePatternHasGlobMagic(pattern) {
  return patternHasGlobMagic(pattern);
}
...
class FileWatcherImpl extends EventEmitter {
  ...
  async _validateConcreteAddPath(file) {
    ...
    if (patternHasGlobMagic(file, { magicalBraces: true })) {
      await this._closeAfterFailedAdd(
        new TypeError(
