...
Server.prototype = {
    initServer: function(){
        ...
        this.exp.configure(function(){
            ...
            var self = this
            if(self.config.static_content){
                var routes = this.config.static_content
                routes.forEach(function(route){
                    var url = route[0]
                    var path = route[1]
                    log.info('adding route:' + url + 
                             ' mapped to ' + path)
                    self.exp.use(url, Express.static(path))
              })
            }
        }.bind(this))
    ...
    monitorFiles: function(callback){
        ...
        // For dynamically loaded JavaScript files
        // track all the files in the defined routes
        var self = this
        if(self.config.static_content){
            var routes = this.config.static_content
            routes.forEach(function(route){
                var path = route[1]
                log.info('Looking to track files in:' + route)
                glob(path+'/**/*.js', function(err, files){
                    if(err) {
                        log.error('Failed to glob path: ' + path)
                        return
                    }
                    files.forEach(function(file){
                        log.info('Tracking file:' + file)
                        self.trackFile.bind(file)
                    })
                })
            })
        }
