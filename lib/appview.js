...
var charm = initCharm()
  ...
  , indent = require('./strutils').indent
...
var SplitLogPanel = exports.SplitLogPanel = View.extend({
    ...
    , scrollUp: function(){
        ...
        }else if (runner.hasResults() && !runner.hasMessages()){
            this.topPanel.scrollUp()
        }else if (this.get('focus') === 'top'){
            this.topPanel.scrollUp()
        }else{
            this.bottomPanel.scrollUp()
        }
...
var ErrorMessagesPanel = exports.AppView = ScrollableTextPanel.extend({
    initialize: function(attrs){
    ...
    , render: function(){
        ...
        for (var l = line + 1; l < line + height; l++){
            charm.position(col, l)
            charm.write(Chars.vertical)
            charm.position(col + width, l)
            charm.write(Chars.vertical)
        }

        charm.position(col, line + height)
