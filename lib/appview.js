...
var LogPanel = exports.LogPanel = View.extend({
    ...
    , wrapText: function(lines){
        ...
        while(idx < lines.length){
            ...
            if (line.length > cols){
                ...
                var leadingWhitespace = line.match(/^ */)
    ...
    , updateText: function(){
        var lines = this.getLogTextLines()
        this.wrapText(lines)
        try{
    ...
    , getLogTextLines: function(){
        //if (this.browser.get('type') === 'browser')
        return this.getLogTextFromTestResults()
        //else
            //return this.getLogTextFromLogOutputChunks().toArray()
    }
    , getLogTextFromTestResults: function(){
    ...
    , renderConsoleOutput: function(firstOrLast){
        try{
            log.info('renderConsoleOutput')
            var charm = this.charm
              , browser = this.browser
              , line = this.get('line')
              , col = this.get('col')
              , lines = browser.get('logOutputChunks')
              , height = browser.hasResults() ? this.get('height') / 2 : this.get('height')
              , width = this.get('width')
              , vertScrollOffset = this.get('vertScrollOffset')

            if (!this.hasLogMessages()) return
            if (!height) return
            
            function renderLine(i){    
                var lineObj = lines ? lines.at(i) : null
                var textLine = (lineObj ? lineObj.get('text') : '').substring(0, width)
                //log.info(lineObj.get('source') + ': ' + lineObj.get('text'))
                if (lineObj && lineObj.get('type') === 'error'){
                    charm.foreground('red')
                }else{
                    charm.foreground('yellow')
                }
                charm
                    .position(col, line + i + (browser.hasResults() ? height : 0))
    ...
    , renderTestResults: function(firstOrLast){
        var charm = this.charm
          , line = this.get('line')
          , col = this.get('col')
          , lines = this.getLogTextLines()
        ...
        // to implement split screen, we divide height into 2
        height = this.hasLogMessages() ? height / 2 - 1 : height
    ...
    , render: function(firstOrLast){
        ...
        var charm = this.charm
          ...
          , height = this.get('height') / 2 - 1
          , width = this.get('width')
          , browser = this.get('browser')
        this.renderTestResults(firstOrLast)

        if (browser.hasResults() && this.hasLogMessages()){
            // draw the dividing line
            charm
                .position(0, line + height)
                .write(Array(width + 1).join(Chars.dot))
                .display('reset')
        }

        this.renderConsoleOutput(firstOrLast)
    }
})
