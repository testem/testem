...
var BrowserTab = exports.BrowserTab = View.extend({
    ...
    , initialize: function(browser, index, appview){
        ...
        // TODO rewrite this part using a helper
        appview.on('change:currentTab', this.on_change_currentTab = function(){
            log.info('current tab changed to ' + appview.get('currentTab') + ', my (' + 
                tab.get('browser').get('name') + ') idx ' + tab.get('index'))
            tab.set('selected', appview.get('currentTab') === tab.get('index'))
        })
        browser.on('change:name', this.on_change_name = function(){
            log.info('browser name changed' + browser.get('name'))
            tab.renderBrowserName()
        })
        browser.get('results').on('change add remove', this.on_add_remove = function(){
            tab.renderResults()
        })
        this.on('change:index', this.on_change_index = function(){
            tab.render()
        })
        this.on('change:selected', this.on_change_selected = function(){
            tab.renderTab()
        })
...
var BrowserTabs = exports.BrowserTabs = Backbone.Collection.extend({
    ...
    , initialize: function(tabs, options){
        ...
        this.appview.browsers().on('remove', function(removed, browsers, options){
            ...
            log.info('removed browser at ' + idx)
            var tab = self.at(idx)
        ...
        this.on('remove', function(removed, tabs, options){
            ...
            if (currentTab >= self.length){
                currentTab--
                log.info('setting currentTab to ' + currentTab)
    ...
    , renderLine: function(){
        ...
        log.info('length: ' + this.length + ', startCol: ' + startCol)
        this.charm
...
var AppView = exports.AppView = View.extend({
    initialize: function(app){
        ...
        this.app = app
        this.initCharm()
        var browserTabs = new BrowserTabs([], {appview: this})
        this.set({
            currentTab: -1
        ...
        browsers.on('add remove', function(){
            log.info('browsers length changed to ' + browsers.length)
        ...
        ;(function checkTermSize(){
            getTermSize(function(cols, lines){
                if (cols !== self.get('cols') || lines !== self.get('lines')){
                    log.info('Terminal size changed to ' + cols + ', ' + lines + '. Reset settings.')
                    self.set({
                        cols: cols
                        , lines: lines
                    })
