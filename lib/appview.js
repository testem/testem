...
AppView.prototype = {
    ...
    renderTabs: function(){
        ...
        this.blankOutLine(6)
        charm.position(1, 7)
        charm.write(Array(this.cols).join('\u2501'))
        browsers.forEach(function(browser, idx){
            ...
            if (selected){
                ...
                charm.write((firstCol ? BoxChar.horizontal : BoxChar.topLeft) +
                    Array(colWidth - 1).join(BoxChar.horizontal) + 
                        BoxChar.topRight)
                ;[5, 6].forEach(function(row){
                    if (!firstCol){
                        charm.position(startCol, row)
                        charm.write(BoxChar.vertical)
                    }
                    charm.position(startCol + colWidth - 1, row)
                    charm.write(BoxChar.vertical)
                })

                var bottomLine = (firstCol ? ' ' : BoxChar.bottomLeft) +
                    Array(colWidth - 1).join(' ') + BoxChar.bottomRight
