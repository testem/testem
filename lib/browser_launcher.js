...
function browsersForPlatform(tempDirFromOptions){
  var platform = process.platform;
  if (tempDirFromOptions) {
      try {
          fs.lstatSync(tempDirFromOptions).isDirectory()
          tempDir = tempDirFromOptions;
      }
      catch(err) {
          console.log('Path that was set in option tempDir is invalid and will be ignored', err);
      }
  }
  if (platform === 'win32'){
    return  [
...
function getAvailableBrowsers(cb, tempDir){
  var browsers = browsersForPlatform(tempDir)
