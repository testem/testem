...
function browserExeExists(cb){
  ...
  if (browser.exe instanceof Array)
    async.filter(browser.exe, fileExists, function(exes){
      if (exes.length > 0)
        browser.exe = exes[0] // commit to the first match
      cb(exes.length > 0)
    })
  else
...
function findableByWhich(cb){
  ...
  if (browser.exe instanceof Array)
    async.filter(browser.exe, which, function(exes){
      if (exes.length > 0)
        browser.exe = exes[0] // commit to the first match
      cb(exes.length > 0)
    })
  else
    exec('which ' + browser.exe, function(err, exePath){
      cb(!!exePath)
    })
}
  ...
  if (platform === 'win32'){
    ...
    return [
      ...
      },
      {
        ...
        setup: function(app, done){
          var host = app.config.get('host')
          fs.writeFile(tempDir + '/testem.safari.html', "<script>window.location = 'http://" + host + ":" + app.config.get('port') + "/'</script>", done)
        },
        args: function(app){
