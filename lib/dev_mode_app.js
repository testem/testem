...
App.prototype = {
    ...
    , configure: function(cb){
        ...
        config.read(function(){
            ...
            config.getSrcFiles(function(err, files){
                self.watchFiles(_(files).pluck('src'))
            })
            if (cb) cb.call(self)
        })
    }
    ...
    , onFileChanged: function(filepath){
        ...
        if ((configFile && filepath === Path.resolve(configFile)) ||
            ...
            var url = Path.relative(process.cwd(), filepath).replace(/\\/g, '/')
            this.startTests(null, url)
        }
    }
    , onEMFILE: function(){
        var view = this.view
    ...
    , startTests: function(callback, data){
        try{
            var view = this.view
