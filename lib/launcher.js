...
const { fromCallback } = require('./utils/promises');
const { registerCleanup } = require('./utils/tmp-cleanup');
...
module.exports = class Launcher {
  ...
  launch() {
    ...
    const setupPromise = settings.setup
      ? fromCallback(setupCallback => settings.setup.call(this, this.config, setupCallback))
      : Promise.resolve();

    return setupPromise.then(() => this.doLaunch());
  }
  ...
  doLaunch() {
    ...
    // GUI browsers must not inherit the TUI's raw stdin (terminal-kit grabInput).
    ...
    if (settings.exe) {
      ...
      return Promise.reject(new Error('No command or exe/args specified for launcher ' + this.name));
    }
  }
  ...
  setupBrowserTmpDir() {
    ...
    const dir = fs.mkdtempSync(tmpPath);
    registerCleanup(dir);
    this.browserTmpDirectory = dir;
  }
  ...
  browserTmpDir() {
    ...
    return this.browserTmpDirectory;
  }
};
