...
Launcher.prototype = {
  ...
  listeners: function(eventName) {
    return this.processCtl.listeners(eventName);
  },
  doLaunch: function(cb) {
    ...
    if (settings.exe) {
      ...
      return this.processCtl.spawn(settings.exe, args, options).asCallback(cb);
    } else if (settings.command) {
      var cmd = this.template(settings.command);
      log.info('cmd: ' + cmd);

      return this.processCtl.exec(cmd, options).asCallback(cb);
    }
  },
  getUrl: function() {
  ...
  kill: function(sig, cb) {
    return this.processCtl.kill(sig).asCallback(cb);
  },
