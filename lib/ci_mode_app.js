...
function App(config){
  ...
  config.getLaunchers(this, function(launchers){
    ...
    self.server.once('server-start', function(){
      self.setupOnStartHook(function(){
        self.begin()
      })
    })
  })
...
App.prototype = {
  ...
  , setupOnStartHook: function(callback){
    var self = this
    ...
    if (self.onStartProcess){
      self.onStartProcess.bad(function(err, stdout, stderr){
        var line = {
          ok: false,
          name: 'error from on_start hook',
          stdout: stdout,
          stderr: stderr
        }
        self.tapProducer.write(line)
        process.exit(1)
      })
    }
  }
  ...
  , makeTestResultsForUncaughtError: function(errMsg, launcher){
    return this.makeTestResults({
      passed: false
      , testName: ''
      , errMsg: errMsg
  ...
  , getRacers: function(launcher){
    ...
    if (config.get('bail_on_uncaught_error')){
      racers.push(function(done){
        self.once('top-level-error', function(msg, url, line){
          log.info('got top-level-error')
          var message = msg + ' at ' + url + ', line ' + line + '\n'
          var results = self.makeTestResultsForUncaughtError(message, launcher)
          done(results)
        })
      })
    }
  ...
  , onBrowserLogin: function(browserName, client){
    var browserRunner = this.connectBrowser(browserName, client)
    browserRunner.on('test-result', this.onTestResult.bind(this))
    var self = this
    browserRunner.on('tests-end', function(results){
      self.emit('all-test-results', browserRunner.get('results'))
    })
  }
  , outputTap: function(results, launcher){
    ...
    if (config.get('fail_on_zero_tests') && tests.length === 0) {
      this.failed = true
