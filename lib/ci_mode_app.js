...
App.prototype = {
    ...
    , runAllTheTests: function(){
        ...
        async.forEachSeries(this.launchers, function(launcher, next){
            ...
            race(self.getRacers(launcher), function(results, gotResults){
                ...
                var topLevelError  = results.get('topLevelError')
                if (topLevelError) {
                    console.log("\n# " + topLevelError)
                    console.log("# Bailing out")
                    self.tapProducer.writable = false
                } else {
    ...
    , getRacers: function(launcher){
        ...
        return [
            ...
            , function(done){
                self.once('top-level-error', function(results) {
                    done(results, true)
                })
            }
    ...
    , quit: function(err){
        var self = this
        if (this.tapProducer && this.tapProducer.writable){
