...
//var log = new (require('log'))('info', fs.createWriteStream('testem.log2'))
...
Server.prototype = {
  ...
  configureProxy: function(app) {
    ...
    var self = this;
    if (proxies) {
        self.proxy = new httpProxy.RoutingProxy({changeOrigin: true})

        for (var proxy in proxies) {
            app.all(proxy + '/*', function(req, res) {
                self.proxy.proxyRequest(req, res, {
                    host: proxies[proxy].host,
                    port: proxies[proxy].port
                  })
            })
        }
    }
