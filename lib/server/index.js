...
Server.prototype = {
  ...
  , configurePlugins: function(app){
    var config = this.config

    var plugins = config.get('plugins') || []
    for (var i=0; i < plugins.length; i++){
      var pluginConfig = plugins[i];
      this.configurePlugin(pluginConfig, app)
    }
  }
  , configurePlugin: function(pluginConfig, app){
    try {
      var path = this.resolvePluginPath(pluginConfig.path)
      var plugin = require(path)
      plugin.init(pluginConfig, app)
    } catch (error){
      log.error('Plugin not configured', pluginConfig)
      log.error('Error was', error)
      log.error('Server directory', path)
    }
  }
  , resolvePluginPath: function(path){
    if (path.indexOf('/') === -1){
      return path
    } else {
      return this.config.resolvePath(path)
    }
  }
