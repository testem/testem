...
function TapMessagesCollector(runner){
  this.runner = runner
  this.results = runner.get('results')
  var tapConsumer = new BrowserTapConsumer(runner.get('socket'))
  ...
  tapConsumer.on('all-test-results', function(){
    ...
    runner.trigger('all-test-results', runner.get('results'))
  })
}
