...
const { fromCallback, disposer } = require('../utils/promises');
...
module.exports = class HookRunner {
  ...
  run(hook, data) {
    ...
    if (command) {
      ...
      return Promise.reject(new Error('No command or exe/args specified for hook ' + hook));
    }
  ...
  static with(config, hook, data) {
    ...
    let runPromise = hookRunner.run(hook, data).catch(err => {
      log.error(hook + ' error', err);

      throw err;
    });

    return disposer(runPromise, () => hookRunner.stop());
  }
};
