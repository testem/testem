...
const { asCallback } = require('../utils/promises');
...
module.exports = class BrowserTestRunner {
    ...
    return new Promise(resolve => {
      ...
      if (this.socket) {
        ...
        this.launcher.start().then(browserProcess => {
          ...
          if (!this.launcher.settings.ignoreProcessExit) {
  ...
  tryAttach(browser, id, socket) {
    ...
    socket.on('tap-debug', function(data) {
      if (process.env.TESTEM_VERBOSE) {
        log.verbose('tap-debug', data && data.msg);
      }
    });
