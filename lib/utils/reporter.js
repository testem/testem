...
class Reporter {
  ...
  setErrorPopupMessage(msg) {
    const reporter = this.reporters.find(function(r) {
      return r && typeof r.setErrorPopupMessage === 'function';
    });
    if (reporter) {
      reporter.setErrorPopupMessage(msg);
    }
  }
}

Reporter.with = (app, stdout, path) => disposer(Promise.resolve(new Reporter(app, stdout, path)), (reporter, promise) => {
  if (promise.isRejected()) {
    let err = promise.reason();

    if (reporter && !err.hideFromReporter) {
  ...
  return reporter ? reporter.close() : Promise.resolve();
});
