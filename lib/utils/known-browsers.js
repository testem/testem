  ...
  var browsers = [
    {
      ...
      args: function(config, url) {
        var profileDir = config.get('firefox_profile');
        if (!profileDir || !fs.statSync(profileDir).isDirectory()) {
  	      profileDir = this.browserTmpDir();
        }
  	    return ['-profile', profileDir, url];
      },
      setup: function(config, done) {
        if( config.get('firefox_profile')) {
          // we done() here so we don't overwrite the prefs.js from the specified profile
            done();
        } else {
          ...
          var prefs = [
            ...
            'user_pref("browser.cache.disk.smart_size.first_run", false);',
  	       ...
  	       fs.writeFile(path.join(firefoxProfileDirectory(this.browserTmpDir(),config), 'prefs.js'), prefs.join('\n'), done);
        }
      }
    },
    {
