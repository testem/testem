...
function firefoxSetup(browserTmpDir, config, done) {
  ...
  if (config.get('firefox_user_js')) {
    return fs.promises.readFile(config.get('firefox_user_js'))
      .then(content => fs.promises.writeFile(perfJS, content))
      .then(() => done(null), done);
  }

  // using user.js to suppress the check default browser popup,
  // the welcome start page, and first-run helper installation prompts
  let prefs = [
  ...
  fs.promises.writeFile(perfJS, prefs.join(os.EOL))
    .then(() => done(null), done);
}
