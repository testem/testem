...
function headlessChromeArgs(browserTmpDir, url) {
  ...
  ].concat(chromeArgs(browserTmpDir, url));
}

function braveWinPaths(homeDir, name) {
  return [
    homeDir +
      '\\Local Settings\\Application Data\\BraveSoftware\\' +
      name +
...
function headlessBraveArgs(browserTmpDir, url) {
  return [
    ...
    '--remote-debugging-port=0',
    '--window-size=1440,900',
  ].concat(braveArgs(browserTmpDir, url));
}
...
function firefoxSetup(browserTmpDir, config, done) {
  ...
  if (config.get('firefox_user_js')) {
    return fs.promises
      .readFile(config.get('firefox_user_js'))
      .then((content) => fs.promises.writeFile(perfJS, content))
      .then(() => done(null), done);
  }

  // user.js: legacy Testem defaults plus prefs aligned with common Firefox --pref automation
  // sets (welcome, onboarding, messaging, telemetry) and fewer prompts / background noise.
  let prefs = [
  ...
  fs.promises
    .writeFile(perfJS, prefs.join(os.EOL))
    .then(() => done(null), done);
}
...
module.exports = function knownBrowsers(platform, config) {
  ...
  let browsers = [
    ...
    {
      name: 'Brave',
      possiblePath: braveWinPaths(userHomeDir, 'Brave-Browser').concat(
        braveOSXPaths('Brave Browser'),
      ),
      possibleExe: ['brave'],
      args(config, url) {
    ...
    {
      name: 'Chrome',
      possiblePath: chromeWinPaths(userHomeDir, 'Chrome').concat(
        chromeOSXPaths('Google Chrome'),
      ),
      possibleExe: ['google-chrome', 'google-chrome-stable', 'chrome'],
      args(config, url) {
    ...
    // On Windows, Chrome Stable/Beta/Dev are installed on the same path
    {
      ...
      possibleExe: ['google-chrome-beta'],
      args(config, url) {
        return chromeArgs(this.browserTmpDir(), url);
      },
    },
    ...
    },
    {
      ...
      possibleExe: ['google-chrome-unstable'],
      args(config, url) {
        return chromeArgs(this.browserTmpDir(), url);
      },
    },
    ...
    {
      name: 'Chrome Canary',
      possiblePath: chromeWinPaths(userHomeDir, 'Chrome SxS').concat(
        chromeOSXPaths('Google Chrome Canary'),
      ),
      args(config, url) {
        return chromeArgs(this.browserTmpDir(), url);
      },
    ...
    {
      name: 'Chromium',
      possiblePath: chromeWinPaths(userHomeDir, 'Chromium').concat(
        chromeOSXPaths('Chromium'),
      ),
      possibleExe: ['chromium-browser', 'chromium'],
      args(config, url) {
        return chromeArgs(this.browserTmpDir(), url);
      },
    },
    ...
    platform === 'darwin'
      ? {
          // Use Launch Services so Safari opens the http(s) URL directly. Passing a local
          // start.html file makes Safari 13+ prompt for confirmation (#1387) and breaks CI.
          name: 'Safari',
          possiblePath: '/usr/bin/open',
          // `open` exits right after handing off to Safari; Testem must not treat that as the
          // browser process dying (browser_test_runner ignoreProcessExit).
  ...
  if (isWin(platform)) {
    ...
    browsers.push({
      // Same Launch Services approach as Safari on macOS (#1387).
      name: 'Safari Technology Preview',
      possiblePath: '/usr/bin/open',
      ignoreProcessExit: true,
      args(config, url) {
        return ['-a', 'Safari Technology Preview', url];
      },
    });
  }
  ...
  if (browserPaths || browserExes) {
    browsers.forEach((browserObject) => {
      let browserPath = browserPaths
        ? browserPaths[browserObject.name]
        : undefined;
      let browserExe = browserExes
        ? browserExes[browserObject.name]
        : undefined;
      if (browserPath) {
