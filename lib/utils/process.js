const execa = require('execa').execa;
const log = require('../log');
const EventEmitter = require('events').EventEmitter;

const isWin = require('./is-win')();
const closeTimeoutAfterExit = 1000;

module.exports = class Process extends EventEmitter {
  constructor(name, options, process) {
    super();

    this.name = name;
    this.killTimeout = options.killTimeout;
    this.process = process;
    this.stdout = '';
    this.stderr = '';
    this.exitAt = null;
    this._killResolve = null;
    this._closeFallbackTimer = null;
    if (this.process.stdout) {
      this.process.stdout.on('data', chunk => {
        this.stdout += chunk;
        this.emit('out');
      });
    }
    if (this.process.stderr) {
      this.process.stderr.on('data', chunk => {
        if (chunk.indexOf('[warn] kevent: Invalid argument') >= 0) {
          return;
        }
        this.stderr += chunk;
      });
    }

    this.process.on('close', this.onClose.bind(this));
    this.process.on('exit', code => {
      this.exitAt = Date.now();
      this.exitCode = code;
      this.scheduleCloseFallback();
    });
    this.process.on('error', this.onError.bind(this));

    this.process.then(result => {
      if (result.failed && result.exitCode !== 0 &&
        result.shortMessage &&
        result.shortMessage.includes(isWin ? 'is not recognized' : 'ENOENT'))
      {
        this.onError(new Error(result.shortMessage), this.stdout, this.stderr);
      }
      return Promise.resolve(result);
    });
  }

  scheduleCloseFallback() {
    if (!this._killResolve || !this.exitAt || this._closeFallbackTimer || !this._killTimer) {
      return;
    }

    this._closeFallbackTimer = setTimeout(() => {
      this._closeFallbackTimer = null;

      if (!this._killResolve || !this.exitAt) {
        return;
      }

      if (this._killTimer) {
        clearTimeout(this._killTimer);
        this._killTimer = null;
      }

      this.process = null;

      const resolve = this._killResolve;
      this._killResolve = null;
      resolve(this.exitCode);
    }, closeTimeoutAfterExit);
  }

  onKillTimeout() {
    log.warn('Process ' + this.name + ' not terminated in ' + this.killTimeout + 'ms.');
    kill(this.process, 'SIGKILL');
    this.scheduleCloseFallback();
  }

  onClose(code) {
    if (!this.process) {
      return;
    }
    log.warn(this.name + ' closed', code);
    this.process = null;
    this.exitCode = code;
    this.emit('processExit', code, this.stdout, this.stderr);
  }

  onError(error) {
    log.warn(this.name + ' errored', error);
    // this.process = null;
    this.exitCode = 1;
    this.emit('processError', error, this.stdout, this.stderr);
  }

  onStdOut(pattern, fn, timeout) {
    let self = this;
    let timeoutID;

    let listener = () => {
      if (self.patternMatches(pattern)) {
        if (timeoutID) {
          clearTimeout(timeoutID);
        }
        return fn(null, self.stdout, self.stderr);
      }
    };

    this.on('out', listener);

    if (timeout) {
      timeoutID = setTimeout(() => {
        self.removeListener('out', listener);
        return fn(new Error('Timed out without seeing "' + pattern + '"'), self.stdout, self.stderr);
      }, timeout);
    }
  }

  patternMatches(pattern) {
    if (typeof pattern === 'string') {
      return this.stdout.indexOf(pattern) !== -1;
    } else { // regex
      return !!this.stdout.match(pattern);
    }
  }

  kill(sig) {
    if (!this.process) {
      log.info('Process ' + this.name + ' already killed.');

      return Promise.resolve(this.exitCode);
    }

    if ((this.exitAt !== null && this.exitAt !== undefined) || this.process.killed ||
      (this.process.exitCode !== null && this.process.exitCode !== undefined)) {
      log.info('Process ' + this.name + ' already exited.');
      // The child is gone but its stdio may still be open, so 'close' may never
      // arrive. Report the exit now; runners wait on processExit to finish up.
      const code = this.exitCode === null || this.exitCode === undefined
        ? this.process.exitCode
        : this.exitCode;
      this.onClose(code);
      return Promise.resolve(this.exitCode);
    }

    sig = sig || 'SIGTERM';

    let self = this;

    return new Promise(resolve => {
      self._killResolve = resolve;
      self.process.once('close', (code, sig) => {
        if (self._closeFallbackTimer) {
          clearTimeout(self._closeFallbackTimer);
          self._closeFallbackTimer = null;
        }
        self._killResolve = null;
        self.process = null;
        if (self._killTimer) {
          clearTimeout(self._killTimer);
          self._killTimer = null;
        }
        log.info('Process ' + self.name + ' terminated.', code, sig);

        resolve(code);
      });
      self.process.on('error', err => {
        log.error('Error killing process ' + self.name + '.', err);
      });
      self._killTimer = setTimeout(self.onKillTimeout.bind(self), self.killTimeout);

      // Check for self.process to make sure the process is still available.
      // Otherwise don't do anything.
      // See https://github.com/testem/testem/pull/1335 for more information.
      if (self.process) {
        kill(self.process, sig);
        self.scheduleCloseFallback();
      } else {
        log.info('Process ' + this.name + ' already killed.');
      }
    });
  }
};

// Kill process and all child processes cross platform
function kill(p, sig) {
  if (isWin) {
    let command = 'taskkill.exe';
    let args = ['/t', '/pid', p.pid];
    if (sig === 'SIGKILL') {
      args.push('/f');
    }

    const killProcess = execa(command, args, { reject: false });

    killProcess.on('error', err => {
      log.error(err);
    });

    killProcess.then(result => {
      // Processes without windows can't be killed without /F, detect and force
      // kill them directly
      if (result.stderr.indexOf('can only be terminated forcefully') !== -1) {
        kill(p, 'SIGKILL');
      }
    });
  } else {
    p.kill(sig);
  }
}
