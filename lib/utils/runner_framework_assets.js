...
function resolveJasmine(dirs, missing) {
  ...
  const jasmineSplitBoot = hasCore && hasBoot0 && hasBoot1;
  const jasmineLegacy = hasCore && hasLegacyBoot && !jasmineSplitBoot;
  ...
  } else if (!jasmineSplitBoot && !jasmineLegacy) {
    reportMissing(missing, {
      install: 'jasmine-core',
  ...
  return {
    ...
    jasmineBoot: jasmineSplitBoot ? null : nodeModulesUrl(JASMINE_BOOT)
  };
}
