...
async function using(resource, fn) {
  ...
  if (isDisposer) {
    try {
      ...
      try {
        await resource.cleanup(undefined, inspection);
      } catch {
        // Prefer the setup error below
      }
      throw setupErr;
    }
  } else {
...
function asCallback(cb) {
  return [
    result => { if (cb) { cb(null, result); } return result; },
    err => { if (cb) { cb(err); } throw err; }
  ];
}

module.exports = { fromCallback, filter, reduce, each, mapLimit, retry, delay, asCallback, Disposer, disposer, using };
