...
module.exports = class App extends EventEmitter {
  ...
  start(cb) {
    ...
    return using(SignalListeners.with(), signalListeners => {
      ...
      return using(Reporter.with(this, this.stdoutStream, this.reportFileName), reporter => {
    ...
    }).then(() => this.cleanExit(), err => this.cleanExit(err));
  }
  ...
  waitForTests() {
    ...
    if (this.exited) {
      return Promise.reject(this.exitErr || new Error('Testem exited before running any tests.'));
    }
  ...
  getServer() {
    return disposer(Promise.resolve(this.startServer()), () => this.stopServer());
  }
  ...
  getRunners() {
    return disposer(fromCallback(callback => {
  ...
  stopRunners() {
    return Promise.all(this.runners.map(runner => {
