...
module.exports = class App extends EventEmitter {
  ...
  startServer() {
    ...
    // The dev reporter waits for this before taking over the terminal, so a
    // failure to bind still reaches stderr instead of a half-drawn dashboard.
  ...
  configureFileWatch() {
    ...
    return FileWatcher.create(this.config).then((fw) => {
      this.fileWatcher = fw;
      ...
      this.fileWatcher.on('EMFILE', () => {
        ...
        this.reporter.setErrorPopupMessage(text);
      });
    });
  }
