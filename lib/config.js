...
class Config {
  ...
  getTestPage() {
    ...
    return testPage.map(page => self.mergeUrlAndQueryParams(page, queryParams));
  }
  ...
  getAvailableLaunchers(cb) {
    ...
    browser_launcher.getAvailableBrowsers(self, browsers, (err, availableBrowsers) => {
      if (err) {
        return cb(err);
      }
  ...
  getFileSet(want, dontWant, callback) {
    ...
    dontWant = dontWant.map(p => p ? self.resolvePath(p) : p);
    Bluebird.reduce(positiveWants, (allThatIWant, patternEntry) => {
      ...
      return globAsync(self.resolvePath(pattern), { ignore: dontWant }).then(files => allThatIWant.concat(files.map(f => {
