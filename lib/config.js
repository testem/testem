...
const expandGlobPattern = require('./utils/expand_glob_pattern');
...
class Config {
  ...
  read(callback) {
    ...
    if (configFile) {
      ...
      return filter(
        files.map(this.resolveConfigPath.bind(this)),
        fileExists,
      ).then((matched) => {
  ...
  readConfigFile(configFile, callback) {
    if (!configFile) {
      // allow empty configFile for programmatic setups
      if (callback) {
        callback.call(this);
      }
  ...
  readYAML(configFile, callback) {
    return fs.promises
      ...
      .then(
        (data) => {
          this.fileOptions = yaml.load(String(data));
        },
        () => {},
      )
      .then(() => {
  ...
  readJSON(configFile, callback) {
    return fs.promises
      ...
      .then(
        (data) => {
          this.fileOptions = JSON.parse(data.toString());
  ...
  getTestPage() {
    ...
    return testPage.map((page) =>
      this.mergeUrlAndQueryParams(page, queryParams),
    );
  }
  ...
  getConfigProperty(key) {
    ...
    if (
      key in this.progOptions &&
      typeof this.progOptions[key] !== 'undefined'
    ) {
      return this.progOptions[key];
    }
    if (
      key in this.fileOptions &&
      typeof this.fileOptions[key] !== 'undefined'
    ) {
      return this.fileOptions[key];
    }
    if (
      this.defaultOptions &&
      key in this.defaultOptions &&
      typeof this.defaultOptions[key] !== 'undefined'
    ) {
  ...
  getAvailableLaunchers(cb) {
    ...
    browser_launcher.getAvailableBrowsers(
      this,
      browsers,
      (err, availableBrowsers) => {
        if (err) {
          return cb(err);
        }
        ...
        if (customLaunchers) {
          for (let name in customLaunchers) {
            let newLauncher = new LauncherFactory(
              name,
              customLaunchers[name],
              this,
            );
  ...
  printLauncherInfo() {
    this.getAvailableLaunchers((err, launchers) => {
      let launch_in_dev = (this.get('launch_in_dev') || []).map((s) =>
        s.toLowerCase(),
      );
      ...
      const launcherColumnWidth = launchers.reduce(
        (acc, current) =>
          acc > current.name.length ? acc : current.name.length,
        0,
      );
      ...
      console.log(
        'Have ' +
          launchers.length +
          ' launchers available; auto-launch info displayed on the right.',
      );
      console.log(); // newline
            ...
            let kind =
              protocol === 'browser'
                ...
                : protocol === 'tap'
                  ...
                  : 'process';
            let dev =
              launch_in_dev.indexOf(launcher.name.toLowerCase()) !== -1
                ? Chars.mark
                : ' ';
            let ci =
              !launch_in_ci ||
              launch_in_ci.indexOf(launcher.name.toLowerCase()) !== -1
                ? Chars.mark
                : ' ';
            return (
              pad(launcher.name, launcherColumnWidth + 2, ' ', 1) +
              pad(kind, 12, ' ', 1) +
              '  ' +
              ci +
              '    ' +
  ...
  getFileSet(want, dontWant, callback) {
    ...
    return reduce(
      positiveWants,
      (allThatIWant, patternEntry) => {
        let pattern = isa(patternEntry, String)
          ? patternEntry
          : patternEntry.src;
        ...
        return expandGlobPattern(
          convertToPosix(this.resolvePath(pattern)),
          dontWant.map(convertToPosix),
        ).then((files) =>
          allThatIWant.concat(
            files.map((f) => {
  ...
  getSrcFiles(callback) {
    ...
    return this.getFileSet(srcFiles, srcFilesIgnore, callback);
  }
  ...
  getFooterScripts(callback) {
    ...
    return this.getFileSet(want, dontWant, callback);
  }

  getServeFiles(callback) {
    let want = this.get('serve_files') || this.get('src_files') || '*.js';
    let dontWant =
      this.get('serve_files_ignore') || this.get('src_files_ignore') || '';
    return this.getFileSet(want, dontWant, callback);
  }
  ...
  getCSSFiles(callback) {
    ...
    return this.getFileSet(want, '', callback);
  }
  ...
  getAllOptions() {
    ...
    const progOpts = this.progOptions;
    // In subcommand modes, progOptions is a plain object from optsWithGlobals() with
    // a _commander reference pointing to the Command instance for option enumeration.
    // In dev mode (no subcommand), progOptions is the Commander program object directly.
    getOptions(
      progOpts && progOpts._commander ? progOpts._commander : progOpts,
    );
    return options;
  }

  async getTemplateData(cb) {
    let ret = {};
    ...
    const replaceSlashes = (f) => ({
      src: f.src.replace(/\\/g, '/'),
      attrs: f.attrs,
    });

    try {
      const [serveFiles, cssFiles, footerScripts] = await Promise.all([
        this.getServeFiles(),
        this.getCSSFiles(),
        this.getFooterScripts(),
      ]);

      ret.serve_files = serveFiles.map(replaceSlashes);
      ret.css_files = cssFiles.map(replaceSlashes);
      ret.footer_scripts = footerScripts;
...
Config.prototype.defaults = {
  ...
  socket_heartbeat_timeout() {
    ...
    let defaultBrowserDisconnectTimeout =
      this.defaults.browser_disconnect_timeout;

    return browserDisconnectTimeout !== defaultBrowserDisconnectTimeout
