...
class Config {
  ...
  read(callback) {
    ...
    if (configFile) {
      ...
      return filter(files.map(this.resolveConfigPath.bind(this)), fileExists).then(matched => {
  ...
  getFileSet(want, dontWant, callback) {
    ...
    return reduce(positiveWants, (allThatIWant, patternEntry) => {
      let pattern = isa(patternEntry, String) ? patternEntry : patternEntry.src;
      ...
      return globAsync(convertToPosix(this.resolvePath(pattern)), { ignore: dontWant.map(convertToPosix) })
        .then(files => files.sort())
        .then(files => allThatIWant.concat(files.map(f => {
...
Config.prototype.defaults = {
  ...
  socket_heartbeat_timeout() {
    ...
    return (browserDisconnectTimeout !== defaultBrowserDisconnectTimeout) ? browserDisconnectTimeout : 20;
  }
};
