...
var ScrollableTextPanel = module.exports = View.extend({
    ...
    , scrollUp: function(){
        ...
        if (scrollOffset > 0){
            screen.enableScroll(line + 1, line + height)
            scrollOffset--
            this.set('scrollOffset', scrollOffset, {silent: true})
            if (process.platform === 'win32'){
                this.render()
            }else{
                screen.position(0, line + 1)
                screen.scrollUp()
                this.render('first')
            }
        }
    ...
    , halfPageUp: function(){
        var height = this.get('height')
        var scrollOffset = this.get('scrollOffset')
        this.scrollTo(Math.max(0, scrollOffset - Math.floor(height / 2)))
    }
    , halfPageDown: function(){
        var height = this.get('height')
        var scrollOffset = this.get('scrollOffset')
        var textLines = this.get('textLines')
        this.scrollTo(Math.min(scrollOffset + Math.floor(height / 2), textLines.length - height))
    }
