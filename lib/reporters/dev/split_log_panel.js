...
function createPanel(appview, attrs) {
  ...
  function geometryChanged(placed) {
    return !lastPlaced ||
      lastPlaced.x !== placed.x ||
      lastPlaced.y !== placed.y ||
      lastPlaced.width !== placed.width ||
      lastPlaced.height !== placed.height;
  }
  ...
  function apply() {
    ...
    if (geometryChanged(placed)) {
      ...
      lastPlaced = placed;
      // setSizeAndPosition pins textBuffer.y to the widget origin, which
      // would jump a live run back to the top of the log on every TAP line.
      box.scrollTo(box.scrollX, box.scrollY, true);
    }
  }
