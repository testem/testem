...
const RunnerTab = (exports.RunnerTab = View.extend({
  ...
  renderSelected() {
    ...
    screen.write(
      (firstCol ? Chars.horizontal : Chars.topLeft) +
...
exports.RunnerTabs = Backbone.Collection.extend({
  ...
  blankOutBackground() {
    ...
    for (let i = 0; i < TabHeight; i++) {
      screen.position(0, TabStartLine + i).write(pad('', cols, ' ', 1));
    }
  },
  ...
  eraseLast() {
    ...
    for (let i = 0; i < height - 1; i++) {
      screen.position(col, line + i).write(new Array(width + 1).join(' '));
    }
